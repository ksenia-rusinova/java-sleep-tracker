package ru.yandex.practicum.sleeptracker.functions;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import ru.yandex.practicum.sleeptracker.FileSleepLogLoader;
import ru.yandex.practicum.sleeptracker.SleepAnalysisResult;

import java.io.IOException;

import static org.junit.jupiter.api.Assertions.assertEquals;

public class AvgSessionDurationFuncTest {
    private FileSleepLogLoader fileSleepLogLoader;

    @BeforeEach
    void setUp() {
        fileSleepLogLoader = new FileSleepLogLoader();
    }

    @Test
    void testAvgSessionDurationFunc() throws IOException {
        fileSleepLogLoader.readLogsFromFile("src/main/resources/sleep_log.txt");

        AvgSessionDurationFunction avgSessionDurationFunction = new AvgSessionDurationFunction();
        SleepAnalysisResult result = avgSessionDurationFunction.apply(fileSleepLogLoader.getListOfSessions());

        assertEquals(345, result.getValue());
        assertEquals("Средняя продолжительность сессии (в минутах)", result.getDescription());
    }

    @Test
    void testAvgSessionDurationFuncWhenInQueryNull() {
        AvgSessionDurationFunction avgSessionDurationFunction = new AvgSessionDurationFunction();
        SleepAnalysisResult result = avgSessionDurationFunction.apply(fileSleepLogLoader.getListOfSessions());

        assertEquals(0, result.getValue());
        assertEquals("Средняя продолжительность сессии (в минутах)", result.getDescription());
    }
}
